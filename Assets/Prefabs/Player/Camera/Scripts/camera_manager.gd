...
var physics_activated = true

func set_camera(camera: Cameras):
	camera_selected = camera

func set_followable(element, physics = true):
...
func _process(delta: float) -> void:
	...
	if not followable is RigidBody3D or !physics_activated:
		cameras_definition(physics_activated, delta)

func _physics_process(delta: float) -> void:
	if followable is RigidBody3D and physics_activated:
